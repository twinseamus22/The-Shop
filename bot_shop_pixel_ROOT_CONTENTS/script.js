...
].map(([id,name,tag,bio])=>({id,name,tag,bio,img:`assets/bots/${id}.png`}));
...
function makeObject(s=world){
  ...
  return {id:s.nextId++,name:`${pick(COLORS)} ${type}`,sprite,category,peculiarity:pec,description:desc,img:`assets/items/${sprite}.png`,created:Date.now()};
}
